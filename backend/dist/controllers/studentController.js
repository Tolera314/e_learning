...
exports.toggleLiveReminder = exports.getWishlist = exports.toggleWishlist = exports.claimCertificate = exports.getStudentHomeMessages = exports.getStudentRecommendations = exports.getSupportTickets = exports.updateStudentProfile = exports.getStudentProfile = exports.getStudentActivity = exports.submitSupportTicket = exports.getMyReviews = exports.submitReview = exports.getStudentProgress = exports.getStudentEnrolledCourses = exports.getStudentCourseDetail = exports.getStudentPastRecordings = exports.getStudentLiveClassesUpcoming = exports.getStudentLiveClassesToday = exports.getStudentDashSummary = exports.recordLessonProgress = exports.toggleFollowInstructor = exports.getSubmissionStatus = exports.getMyAssignments = exports.submitAssignment = exports.getStudentAssignments = exports.getStudentTasks = void 0;
...
const getStudentAssignments = async (req, res) => {
    try {
        ...
        const studentId = req.user.id;
        const isEnrolled = await (0, security_1.validateEnrollment)(courseId, studentId);
        if (!isEnrolled)
...
const submitAssignment = async (req, res) => {
    try {
        ...
        if (!assignment || !(await (0, security_1.validateEnrollment)(assignment.lesson.module.courseId, studentId))) {
...
const getStudentDashSummary = async (req, res) => {
    try {
        ...
        const [enrolledCourses, completedLessons, quizAgg, upcomingLive, enrollments, activeLive, certificatesCount, pendingAssignmentCount, recentActivity, user] = await Promise.all([
            prisma_1.prisma.enrollment.count({ where: { studentId } }),
            prisma_1.prisma.lessonProgress.count({ where: { studentId, isCompleted: true } }),
            prisma_1.prisma.quizSubmission.aggregate({ where: { studentId }, _avg: { score: true } }),
            prisma_1.prisma.liveSession.count({ where: { startTime: { gte: now }, isLive: false } }),
            prisma_1.prisma.enrollment.findMany({ where: { studentId }, select: { progressPercent: true } }),
            prisma_1.prisma.liveSession.findMany({
                where: {
                    isLive: true,
            ...
            prisma_1.prisma.certificate.count({ where: { studentId } }),
        ...
        const activityDates = new Set(recentActivity.map((a) => new Date(a.createdAt).toDateString()));
        let streak = 0;
        ...
        const avgProgress = enrollments.length
            ? enrollments.reduce((sum, e) => sum + Number(e.progressPercent), 0) / enrollments.length
            : 0;
        res.status(200).json({
...
const getStudentLiveClassesToday = async (req, res) => {
    try {
        const studentId = req.user.id;
        const now = new Date();
...
exports.getStudentLiveClassesToday = getStudentLiveClassesToday;
const getStudentLiveClassesUpcoming = async (req, res) => {
    try {
        const studentId = req.user.id;
        const now = new Date();
        const sessions = await prisma_1.prisma.liveSession.findMany({
...
exports.getStudentLiveClassesUpcoming = getStudentLiveClassesUpcoming;
const getStudentPastRecordings = async (req, res) => {
    try {
        const studentId = req.user.id;
        const now = new Date();
...
exports.getStudentPastRecordings = getStudentPastRecordings;
...
exports.getStudentCourseDetail = getStudentCourseDetail;
...
const getStudentProgress = async (req, res) => {
    try {
        ...
        const totalWatchSeconds = lessonProgressList.reduce((s, l) => s + (l.watchTimeSeconds || 0), 0);
        // Aggregate Recent Quiz Scores (Limit to 7 for the chart)
        const recentQuizScores = allSubmissions.slice(0, 7).map((q) => q.score);
        const quizAvg = allSubmissions.length ? allSubmissions.reduce((s, q) => s + q.score, 0) / allSubmissions.length : 0;
        ...
        const courseProgress = enrollments.map((e) => {
            const totalCourseLessons = e.course.modules.reduce((s, m) => s + m.lessons.length, 0);
            const courseQuizzes = allSubmissions.filter((q) => q.quiz?.lesson?.module?.courseId === e.course.id);
            const courseQuizAvg = courseQuizzes.length ? courseQuizzes.reduce((s, q) => s + q.score, 0) / courseQuizzes.length : 0;
            return {
                id: e.course.id,
                name: e.course.title,
                progress: Math.round(Number(e.progressPercent)),
                lessons: lessonProgressList.filter((lp) => lp.isCompleted && lp.lesson?.module?.courseId === e.course.id).length,
        ...
        res.status(200).json({
            overallProgress: enrollments.length ? Math.round(enrollments.reduce((s, e) => s + Number(e.progressPercent), 0) / enrollments.length) : 0,
...
const submitReview = async (req, res) => {
    try {
        ...
        if (!enrollment)
            return res.status(403).json({ error: "You must be enrolled and have ≥50% progress to review" });
        if (Number(enrollment.progressPercent) < 50)
            return res.status(403).json({ error: "Complete at least 50% of the course before reviewing" });
        const existing = await prisma_1.prisma.review.findFirst({ where: { studentId, courseId } });
        let review;
        if (existing) {
            review = await prisma_1.prisma.review.update({ where: { id: existing.id }, data: { rating, comment } });
        }
        else {
            review = await prisma_1.prisma.review.create({ data: { studentId, courseId, rating, comment } });
        }
...
const updateStudentProfile = async (req, res) => {
    try {
        ...
        await prisma_1.prisma.$transaction([
            ...
            prisma_1.prisma.studentProfile.upsert({
                where: { userId },
                create: {
                    userId,
                    educationLevel,
...
const getStudentRecommendations = async (req, res) => {
    try {
        ...
        const myCategories = enrollments.map((e) => e.course.category);
        ...
        const recommendations = await prisma_1.prisma.course.findMany({
            where: {
        ...
        res.status(200).json(recommendations.map((c) => ({
            id: c.id,
            title: c.title,
...
const getStudentHomeMessages = async (req, res) => {
    try {
        const studentId = req.user.id;
...
exports.getStudentHomeMessages = getStudentHomeMessages;
const claimCertificate = async (req, res) => {
    try {
        ...
        // Check existing
        let certificate = await prisma_1.prisma.certificate.findFirst({
        ...
        page.drawRectangle({
            ...
            borderColor: (0, pdf_lib_1.rgb)(0.12, 0.12, 0.12), borderWidth: 2,
        });
        page.drawRectangle({
            x: 35, y: 35, width: width - 70, height: height - 70,
            borderColor: (0, pdf_lib_1.rgb)(0.5, 0.36, 0.96), borderWidth: 1,
        });
        // Header
        page.drawText('ETHIO-DIGITAL ACADEMY', {
            x: width / 2 - 130, y: height - 100, size: 28, font: boldFont, color: (0, pdf_lib_1.rgb)(0.12, 0.12, 0.12)
        });
        page.drawText('CERTIFICATE OF COMPLETION', {
            x: width / 2 - 150, y: height - 140, size: 20, font: boldFont, color: (0, pdf_lib_1.rgb)(0.4, 0.4, 0.4)
        });
        // Body
        page.drawText('This is to certify that', {
            x: width / 2 - 60, y: height - 220, size: 14, font: regularFont, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5)
        });
        page.drawText(student.name.toUpperCase(), {
            x: width / 2 - (student.name.length * 7), y: height - 260, size: 34, font: boldFont, color: (0, pdf_lib_1.rgb)(0.1, 0.1, 0.1)
        });
        page.drawLine({
            start: { x: width / 2 - 200, y: height - 275 },
            end: { x: width / 2 + 200, y: height - 275 },
            thickness: 2, color: (0, pdf_lib_1.rgb)(0.8, 0.8, 0.8)
        });
        page.drawText(`has successfully completed the professional course`, {
            x: width / 2 - 160, y: height - 320, size: 14, font: regularFont, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5)
        });
        page.drawText(course.title, {
            x: width / 2 - (course.title.length * 5), y: height - 355, size: 22, font: boldFont, color: (0, pdf_lib_1.rgb)(0.5, 0.36, 0.96)
        });
        // Signatures
        page.drawText('INSTRUCTOR', { x: 150, y: 150, size: 10, font: boldFont, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
        page.drawText(course.instructor.name, { x: 150, y: 170, size: 14, font: regularFont, color: (0, pdf_lib_1.rgb)(0.1, 0.1, 0.1) });
        page.drawText('DATE', { x: width - 250, y: 150, size: 10, font: boldFont, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
        page.drawText(new Date().toLocaleDateString(), { x: width - 250, y: 170, size: 14, font: regularFont, color: (0, pdf_lib_1.rgb)(0.1, 0.1, 0.1) });
        // Footer / Verification
        page.drawText(`Verification Code: ${certificate.verificationCode}`, {
            x: width / 2 - 100, y: 50, size: 10, font: regularFont, color: (0, pdf_lib_1.rgb)(0.6, 0.6, 0.6)
        });
...
const toggleWishlist = async (req, res) => {
    try {
        ...
        if (existing) {
            ...
            await prisma_1.prisma.wishlist.create({ data: { studentId, courseId } });
...
const getWishlist = async (req, res) => {
    try {
        ...
        const result = wishlist.map((w) => ({
            ...
            totalLessons: w.course.modules.reduce((s, m) => s + m.lessons.length, 0),
...
const toggleLiveReminder = async (req, res) => {
    try {
        const studentId = req.user.id;
        const { sessionId } = req.params;
        const existing = await prisma_1.prisma.liveReminder.findUnique({
            where: { studentId_liveSessionId: { studentId, liveSessionId: sessionId } }
        });
        if (existing) {
            await prisma_1.prisma.liveReminder.delete({ where: { id: existing.id } });
            return res.status(200).json({ reminded: false });
        }
        else {
            await prisma_1.prisma.liveReminder.create({ data: { studentId, liveSessionId: sessionId } });
...
exports.toggleLiveReminder = toggleLiveReminder;
